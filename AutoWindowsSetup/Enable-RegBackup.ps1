...
try {
    ...
    if (Test-Path $regBackupPath) {
        [Logger]::Write("レジストリバックアップディレクトリ: $regBackupPath")
    }
    else {
    ...
    [Logger]::Write("レジストリ設定を変更します...")

    if (-not (Test-Path $regPath)) {
        [Logger]::Write("レジストリパスが存在しないため作成します: $regPath")
    ...
    [Logger]::Write("")
    [Logger]::Write("タスクスケジューラのRegIdleBackupタスクを確認します...")
    ...
    try {
        ...
        if ($task) {
            ...
            elseif ($task.State -eq "Ready") {
                [Logger]::Warning("タスク '$taskName' は既に有効です。")
                }
                else {
                    [Logger]::Write("タスク '$taskName' の状態: $($task.State)")
                }

                # タスク情報を表示
                [Logger]::Write("")
                [Logger]::Write("タスク情報:")
                [Logger]::Write("  タスク名: $taskName")
                [Logger]::Write("  状態: $($task.State)")
                [Logger]::Write("  最終実行: $($task.LastRunTime)")
                [Logger]::Write("  次回実行: $($task.NextRunTime)")
...
'@
        ...
        [Logger]::Write("")
        [Logger]::Warning("注意事項:")
        [Logger]::Warning("- レジストリバックアップは $regBackupPath に保存されます")
        [Logger]::Warning("- バックアップは定期的に自動実行されます")
        [Logger]::Warning("- システムがアイドル状態の時にバックアップが実行されます")
